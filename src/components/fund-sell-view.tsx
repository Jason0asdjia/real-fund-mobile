...
export function FundSellView({ code }: FundSellViewProps) {
  ...
  const maxSellAmount = Math.max(Number(metrics?.amount || 0), 0);
  const amount = Math.min(toNumber(amountInput) || 0, maxSellAmount);
    ...
    <div className="-mx-3 -mb-24 -mt-4 flex h-[calc(100dvh-5.5rem)] flex-col overflow-hidden bg-white text-[#131b2e] md:-mx-4 md:-mb-24 md:-mt-4">
      ...
      <main className="flex-1 overflow-y-auto pb-24">
        ...
        <section className="border-b border-[#e2e7ff] px-3 py-4">
          <div className="mb-2 flex items-center justify-between">
            <label className="text-sm font-bold text-[#131b2e]">减仓金额 (CNY)</label>
          ...
          <div className="mt-3 grid grid-cols-4 gap-2">
            {[1000, 5000, 10000].map((value) => (
              <button key={value} type="button" className="min-h-8 rounded border border-[#d5dbea] bg-white text-xs font-bold text-[#131b2e]" onClick={() => setAmountInput(String(Math.min(value, maxSellAmount)))}>
                {value.toLocaleString("zh-CN")}
              </button>
            ))}
            <button type="button" className="min-h-8 rounded border border-[#d5dbea] bg-white text-xs font-bold text-[#131b2e]" onClick={() => setAmountInput(String(maxSellAmount.toFixed(2)))}>
              MAX
            </button>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 border-t border-[#e2e7ff] bg-white/95 p-3 backdrop-blur">
        <button
          ...
          disabled={!amount || !latestNav || maxSellAmount <= 0}
