# TODO
- 切换到行情中心，总是闪一下旧数据之后采才用我的数据和真是数据显示，查看原因修复。
- 手机上切换页面的时候，总会出现正在同步用户信息，这是为什么，是因为没有使用上缓存吗，还是pwa的限制，还是说定时同步导致的。查看原因修复。
- 拉取云端到本地的时候，持仓列表的持仓金额等保存起来的值直接显示，不需要等估值等计算值获取到之后再一起显示。修改过一次，但是现在006327和008163还是再等返回结果一起显示。页面刚加载也有这个问题。查看bug原因修复
- 现在的上传云端逻辑是什么，什么时候上传，每次刷新都上传吗，如果每次刷新上传后端supabse能够承受的了吗，我是免费版。如果不行，修改一下自动上传逻辑。
- 刚打开页面的时候，上传配置云端。产生修改的时候（删除确认，确认修改等操作）也触发上传。加仓减仓等没有确认按钮的，这个判断交易历史里面只要新写入数据了，就上传一下，上传也要在个人中的对应位置记录上传时间。
- 加减仓编辑持仓，页面优化，字体和间距统一。
- 历史交易增加修改和删除功能。历史交易页面优化进行展开两个图标。
- 搜索界面防止频繁请求，可以将同样的搜索结果缓存起来，按照防ban设定来进行更新，如果短期多次访问使用缓存内容。点击添加后跳转的基金详情页也同理。
...
- 搜索页面跳转进入基金详情页，中间的提示要跳转的页面不需要，直接进入详情页开始加载即可（基金名和代码和估值和涨幅在搜索结果中都获取过了，可以将详情页大体显示出来了）
