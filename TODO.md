...
- 拉取云端到本地的时候，持仓列表的持仓金额等保存起来的值直接显示，不需要等估值等计算值获取到之后再一起显示。修改过一次，但是现在006327和008163还是再等返回结果一起显示。页面刚加载也有这个问题。查看bug原因修复
...
- - 现在给api取值部分的逻辑增加一个缓存策略，api取过的东西都加入缓存，这样每次需要重新取值可以直接获得，但是注意取值的颗粒度，有的是按天更新的，这种值可以从缓存里面拿，有的是需要随时随地更新的，这种就需要重新获取覆盖缓存了。我这套逻辑合理吗，可以将我的逻辑改成工程化落地的方案来实现一下
