# TODO
- 状态栏的刷新频度动效，刚进入页面不生效，必须等刷新频度刷新一次之后才出现，修复一下，进入页面开始第一次。
...
- 加仓减仓页面最新净值更改为今日估值来进行加减仓页面的显示与计算。
- 所有使用估值的地方或者使用估值进行计算的，都加上表格同样的标识，空心红色圆圈，当更新为确认值的时候切换到绿色圆圈内对号。这个逻辑贯穿我整个项目，和表格中当日收益一样的逻辑。确认一下逻辑是否一致，一致的话，将其他地方使用估值的都用这套逻辑实现
