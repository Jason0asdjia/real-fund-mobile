# TODO
- 蛋卷的api删掉，看看有没有其他的无需登录的api获取源，如果有需要登录但是免费的api提供给我我可以给你获取key使用。现在我数据里的后六个基金不能获取到收盘之后的官方路径的值，12点前官方路径肯定会出值的，查看原因。
- 凡是使用官方链路的显示和计算，当获取到之后，后续就不参与刷新获取了。防止获取到之后，后续刷新获取不到回退了。
- 拉取云端到本地的时候，持仓列表的持仓金额等保存起来的值直接显示，不需要等估值等计算值获取到之后再一起显示。修改过一次，但是现在006327和008163还是再等返回结果一起显示。页面刚加载也有这个问题。查看bug原因修复
- 刚打开页面的时候，上传配置云端。产生修改的时候（删除确认，确认修改等操作）也触发上传。加仓减仓等没有确认按钮的，这个判断交易历史里面只要新写入数据了，就上传一下。
